...
export const generateInvoiceDocument = (data: ConventionData): void => {
  ...
  const addText = (
    ...
    const { fontSize = 10, fontStyle = 'normal', color = TEXT_COLOR_DARK, align = 'left', maxWidth } = options || {};
  ...
  const colWidths = [contentWidth * 0.48, contentWidth * 0.12, contentWidth * 0.15, contentWidth * 0.10, contentWidth * 0.15];
  ...
  pdf.text('PU HT', currentX + colWidths[2] - cellPadding - pdf.getStringUnitWidth('PU HT')*pdf.getFontSize()*0.35 , yPos + headerHeight / 2 + 2, {align: 'right'});
  currentX += colWidths[2];
  pdf.text('Taux de TVA', currentX + colWidths[3] - cellPadding - pdf.getStringUnitWidth('Taux de TVA')*pdf.getFontSize()*0.35, yPos + headerHeight / 2 + 2, {align: 'right'});
  currentX += colWidths[3];
  pdf.text('Montant HT', currentX + colWidths[4] - cellPadding*2 - pdf.getStringUnitWidth('Montant HT')*pdf.getFontSize()*0.35 , yPos + headerHeight / 2 + 2, {align: 'right'});
  
  yPos += headerHeight;

  // Table Row
  const rowStartY = yPos;
  let descColX = margin + cellPadding;
  ...
  pdf.setFont('helvetica', 'bold');
  descY = addText(mainItemDesc, descColX, descY, { fontSize: 9, fontStyle: 'bold', maxWidth: colWidths[0] - cellPadding * 2 }) + 1;
  
  pdf.setFont('helvetica', 'normal');
  const clientText = `Client : ${data.client_nom || 'N/A'}`;
  descY = addText(clientText, descColX, descY, { fontSize: 9, maxWidth: colWidths[0] - cellPadding * 2 }) + 0.5;
  
  const participantsText = `Bénéficiaire(s) : ${data.participants || 'N/A'}`;
  descY = addText(participantsText, descColX, descY, { fontSize: 9, maxWidth: colWidths[0] - cellPadding * 2 }) + 0.5;
  
  const conventionText = `Convention n° ${data.numero_convention || 'N/A'} du ${data.date_signature || 'N/A'}`;
  descY = addText(conventionText, descColX, descY, { fontSize: 9, maxWidth: colWidths[0] - cellPadding * 2 }) + 0.5;
  
  const stageText = `Intitulé du stage : ${data.formation_intitule || 'Formation personnalisée'}`;
  descY = addText(stageText, descColX, descY, { fontSize: 9, maxWidth: colWidths[0] - cellPadding * 2 }) + 0.5;
  
  const datesText = `Dates de formation : ${data.formation_dates || 'N/A'}`;
  descY = addText(datesText, descColX, descY, { fontSize: 9, maxWidth: colWidths[0] - cellPadding * 2 }) + 0.5;
  
  const dureeText = `Durée de la formation : ${data.formation_duree || 'N/A'}`;
  descY = addText(dureeText, descColX, descY, { fontSize: 9, maxWidth: colWidths[0] - cellPadding * 2 }) + 0.5;
  
  pdf.setFont('helvetica', 'bold');
  descY = addText('Référence complémentaire :', descColX, descY, { fontSize: 9, fontStyle: 'bold', maxWidth: colWidths[0] - cellPadding * 2 });
  pdf.setFont('helvetica', 'normal');
  ...
  currentX += colWidths[1];
  pdf.text(formatCurrency(data.formation_tarif_ht), currentX + colWidths[2] - cellPadding, rowContentY, { align: 'right' });
  currentX += colWidths[2];
  pdf.text('20 %', currentX + colWidths[3] - cellPadding, rowContentY, { align: 'right' });
  currentX += colWidths[3];
  pdf.text(formatCurrency(data.formation_tarif_ht), currentX + colWidths[4] - cellPadding*2, rowContentY, { align: 'right' });
  ...
  const addTotalLine = (label: string, value: string, isBoldValue: boolean = false) => {
    ...
    const labelWidth = pdf.getStringUnitWidth(label) * pdf.getFontSize() / pdf.internal.scaleFactor * pdf.internal.scaleFactor * 0.3527777778 * 2.8346456693; // mm
    const valueWidth = pdf.getStringUnitWidth(value) * pdf.getFontSize() / pdf.internal.scaleFactor * pdf.internal.scaleFactor * 0.3527777778 * 2.8346456693; // mm
    ...
    pdf.setDrawColor(BORDER_COLOR);
    pdf.line(totalsXAnchor - (colWidths[2] + colWidths[3] + colWidths[4]), yPos + 4, totalsXAnchor, yPos + 4);
  };
  ...
  // --- Payment Info ---
  if (yPos > 200) { // Check if new page is needed
      pdf.addPage();
      yPos = margin;
  }
  yPos += 10;
  ...
  yPos += 5;
  pdf.setFont('helvetica', 'normal');
  pdf.text('Références bancaires : IBAN : FR76 1660 7004 3340 0010 5706 393 BIC : CCBPFRPPPPG', margin, yPos);
  yPos += 10;
  ...
  pdf.setFontSize(7);
  pdf.setFont('helvetica', 'normal');
  const finePrint = "A défaut de paiement comptant à l'échéance, les intérêts de retard seront appliqués suivant les modalités et le taux minimum défini par la loi du 31/12/1992. Taux Refi majoré de 10 points. Indemnité forfaitaire pour frais de recouvrement (sans TVA): 40€";
  ...
  pdf.setFontSize(8);
  const footerCompanyDetails1 = "SARL LAURENT SERRE – 69 rue Jean Bouin 34130 MAUGUIO - Tél. : 06 14 94 40 60 - n° de déclaration : 91340863434 – RCS :";
  const footerCompanyDetails2 = "MONTPELLIER 435292941 N° SIRET : 43529294100034 – Code APE : 4778A - Capital social : 300 000,00 € - TVA intracommunautaire";
